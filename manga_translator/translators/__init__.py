...
async def dispatch(chain: TranslatorChain, queries: List[str], config: Config, use_mtpe: bool = False, args:Optional[Context] = None, device: str = 'cpu') -> List[str]:
    ...
    # Detect source language for per-language prompt loading
    detected_lang_code = langid.classify('\n'.join(queries))[0]
    detected_source_lang = ISO_639_1_TO_VALID_LANGUAGES.get(detected_lang_code)
    # Use detected language if valid, otherwise fall back to 'auto'
    source_lang = detected_source_lang if detected_source_lang else 'auto'
    
    import logging
    logger = logging.getLogger('manga-translator.dispatch')
    logger.info(f"[Source Language Detection] Detected: {detected_lang_code} -> {source_lang}")
    ...
    if chain.target_lang is not None:
        ...
        for key, lang in chain.chain:           
            ...
            queries = await translator.translate(source_lang, chain.langs[flag], queries, use_mtpe)
            flag+=1
        return queries
    if args is not None:
        args['translations'] = {}
    for key, tgt_lang in chain.chain:
        ...
        if key.value in ["gemini_hq", "openai_hq"]:
            queries = await translator.translate(source_lang, tgt_lang, queries, ctx=args)
        else:
            # Pass ctx parameter for AI line breaking
            queries = await translator.translate(source_lang, tgt_lang, queries, use_mtpe=use_mtpe, ctx=args)
        if args is not None:
