...
重试策略：
        ...
        try:
            ...
            if is_network_error:
                if retry_count < max_network_retries:
                    ...
                    for ctx, config in batch:
                        self.retry_queue.append((ctx, config, 'network_error'))
                    return
            ...
            if is_auth_error:
                if retry_count < max_auth_retries:
                    ...
                    for ctx, config in batch:
                        self.retry_queue.append((ctx, config, 'auth_error'))
                    return
            ...
            if is_safety_error or is_count_mismatch or is_br_error or is_quality_error:
                ...
                if len(batch) > 1:
                    ...
                    for ctx, config in batch:
                        self.retry_queue.append((ctx, config, error_type_str))
                else:
                    # 单张错误：放入延迟队列
                    ctx, config = batch[0]
    ...
    async def _process_retry_queue(self):
        ...
        logger.info(f"[延迟重试] 完成: 成功 {success_count} 张，失败 {fail_count} 张")
