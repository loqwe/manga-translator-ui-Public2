...
class MainWindow(QMainWindow):
    ...
    def on_task_completed(self, saved_files: list):
        ...
        try:
            ...
            if saved_files:
                try:
                    for item in list(saved_files):
                        if isinstance(item, dict) and item.get('results_summary'):
            ...
            file_paths = []
            if saved_files:
                file_paths = [x for x in saved_files if isinstance(x, str) and x]

            self.logger.info(f"on_task_completed called with {len(file_paths)} files")

            summary_line = ""
            if isinstance(results_summary, dict):
                s = int(results_summary.get('success', 0) or 0)
                f = int(results_summary.get('failed', 0) or 0)
                k = int(results_summary.get('skipped', 0) or 0)
                summary_line = self._t("success_summary", success=s, failed=f, skipped=k)
            
            if not file_paths:
                self.logger.info("No saved files, showing completion QMessageBox")
                msg = self._t("translation_completed_no_text")
                if summary_line:
                    msg = summary_line + "\n\n" + msg
            ...
            msg = self._t("translation_completed_with_files", count=len(file_paths))
            if summary_line:
                msg = summary_line + "\n\n" + msg
            ...
            if reply == QMessageBox.StandardButton.Yes:
                ...
                self.enter_editor_mode(files_to_load=file_paths)
        except Exception as e:
