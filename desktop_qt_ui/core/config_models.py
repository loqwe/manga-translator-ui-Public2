...
class CliSettings(BaseModel):
    ...
    translation_concurrency: int = 1  # 翻译并发数
    inpaint_concurrency: int = 1  # 修复并发数
...
class AppSection(BaseModel):
    ...
    monitor_config: Optional[dict] = None  # 监控配置（包含日志展开状态等）
    # 窗口状态
...
class AppSettings(BaseModel):
    ...
    enable_watermark_filter: bool = True  # 是否启用过滤列表
    kernel_size: int = 3
