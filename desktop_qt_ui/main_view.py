...
from widgets.file_list_view import FileListContainer
...
class MainView(QWidget):
    ...
    def _open_filter_list(self):
        ...
        from widgets.filter_list_dialog import FilterListDialog
        
        # 过滤列表文件路径 - 优先尝试相对路径
        filter_path = Path("examples/config/watermark_filter.json")
        
        # 如果相对路径不存在，尝试从当前文件计算绝对路径
        if not filter_path.exists():
            current_file = Path(__file__).resolve()
            base_path = current_file.parent.parent  # desktop_qt_ui -> 项目根目录
            filter_path = base_path / "examples" / "config" / "watermark_filter.json"
        
        # 打开编辑对话框
        dialog = FilterListDialog(self, config_path=str(filter_path))
        dialog.exec()

    def _open_glossary_editor(self):
        """打开术语参考编辑对话框"""
        from pathlib import Path
        from widgets.glossary_filter_dialog import GlossaryFilterDialog
        ...
        if prompt_path:
            # 优先使用配置中的路径
            glossary_path = Path(prompt_path)
        else:
            # 默认使用 dict/prompt_example.json
            glossary_path = Path("dict/prompt_example.json")
        
        # 如果相对路径不存在，尝试从当前文件计算绝对路径
        if not glossary_path.exists():
            current_file = Path(__file__).resolve()
            base_path = current_file.parent.parent  # desktop_qt_ui -> 项目根目录
            glossary_path = base_path / glossary_path
        
        # 打开编辑对话框
        dialog = GlossaryFilterDialog(self, config_path=str(glossary_path))
        dialog.exec()
    ...
    def _create_param_widgets(self, data, parent_layout, prefix=""):
        ...
        for key, value in data.items():
            ...
            # enable_watermark_filter 在 _finalize_settings_ui 中手动创建（带按钮）
            ...
            if full_key == "enable_watermark_filter":
                ...
                usage_btn.clicked.connect(self._open_usage_log)

                glossary_btn = QPushButton(self._t("btn_glossary_log"))
            ...
            elif full_key == "translator.extract_glossary":
                # 特殊处理：自动提取新术语开关 + 编辑术语按钮
                container = QWidget()
                hbox = QHBoxLayout(container)
                hbox.setContentsMargins(0, 0, 0, 0)
                ...
                glossary_btn = QPushButton(self._t("btn_edit_glossary"))
                glossary_btn.clicked.connect(self._open_glossary_editor)
    ...
    def _create_left_panel(self) -> QWidget:
        ...
        self.advanced_folder_button = QPushButton(self._t("Advanced"))
        self.advanced_folder_button.setToolTip(self._t("Advanced folder selection with tree view"))
        ...
        # 文件列表容器（包含文件列表和失败记录）
        self.file_list = FileListContainer(None, self) # Pass None for model initially
        ...
        self.advanced_folder_button.clicked.connect(self._open_advanced_folder)
        ...
        # 连接失败文件记录信号
        self.controller.failed_files_recorded.connect(self.file_list.add_failed_record)
        # 连接失败记录重试信号
        self.file_list.failed_retry_requested.connect(self.controller.retry_failed_record)
        return left_panel
    ...
    def _create_right_panel(self) -> QWidget:
        ...
        try:
            ...
            self.custom_comic_panel = CustomComicPanel()
            self.custom_comic_tab_index = self.settings_tabs.addTab(self.custom_comic_panel, self._t("Custom"))
        except Exception as e:
            self.logger.warning(f"无法加载自定义漫画面板: {e}")
            self.custom_comic_tab_index = -1
        
        # 添加名称映射管理独立标签页
        try:
            from widgets.name_mapping_panel import NameMappingPanel
            self.name_mapping_panel = NameMappingPanel()
            self.name_mapping_tab_index = self.settings_tabs.addTab(self.name_mapping_panel, "名称映射")
        except Exception as e:
            self.logger.warning(f"无法加载名称映射面板: {e}")
            self.name_mapping_tab_index = -1
        
        # 监听标签页切换，自定义标签页时隐藏主日志
        ...
        log_header_layout.setSpacing(5)
        
        log_title = QLabel(self._t("Log output..."))
        log_header_layout.addWidget(log_title)
        log_header_layout.addStretch()
        
        # 日志收缩/展开按钮
        self.log_collapse_btn = QPushButton("▼")
        self.log_collapse_btn.setFixedSize(22, 18)
        self.log_collapse_btn.setToolTip(self._t("Collapse Log"))
        ...
        log_header_layout.addWidget(self.log_collapse_btn)
        ...
        # 日志展开状态
        self._log_expanded = self._monitor_config.get('log_expanded', True)
        ...
        self.right_splitter.setStretchFactor(1, 1) # 日志输出占据较少空间
        
        # 根据保存的展开状态设置初始大小
        if self._log_expanded:
            self.right_splitter.setSizes([400, 400])  # 展开：正常高度
        else:
            self.right_splitter.setSizes([700, 80])  # 收缩：日志保留80px
            self.log_collapse_btn.setText("▲")
            self.log_collapse_btn.setToolTip(self._t("Expand Log"))
    ...
    def refresh_ui_texts(self):
        ...
        if hasattr(self, 'advanced_folder_button'):
            self.advanced_folder_button.setText(self._t("Advanced"))
            self.advanced_folder_button.setToolTip(self._t("Advanced folder selection with tree view"))
    ...
    def _open_usage_log(self):
        from widgets.usage_log_dialog import UsageLogDialog
        dialog = UsageLogDialog(self, translate=self._t)
        dialog.exec()

    def _open_glossary_log(self):
        from widgets.glossary_log_dialog import GlossaryLogDialog
        dialog = GlossaryLogDialog(self, translate=self._t)
        dialog.exec()

    def _open_filtered_log_dialog(self, title: str, keywords: list):
        from widgets.usage_log_dialog import get_latest_ui_log_path

        log_path = get_latest_ui_log_path()
        if not log_path or not os.path.exists(log_path):
            QMessageBox.information(self, title, self._t("No log files found"))
            return
        ...
        lower_keys = [(k or '').lower() for k in (keywords or [])]
        if lower_keys:
            filtered = [
                ln.rstrip('\n')
                for ln in lines
                if any(k in ln.lower() for k in lower_keys)
            ]
        else:
            filtered = [ln.rstrip('\n') for ln in lines]

        content = "\n".join(filtered[:5000]) if filtered else self._t("No matching logs")
    ...
    def _on_test_api_clicked(self, key: str):
        ...
        for env_key in self.env_widgets.keys():
            if "API_BASE" in env_key and "MINUTE" not in env_key:
    ...
    def _open_advanced_folder(self):
        """打开高级文件夹选择面板"""
        try:
            from widgets.advanced_folder_panel import AdvancedFolderDialog
            
            last_dir = self.controller.get_last_open_dir()
            dialog = AdvancedFolderDialog(self, start_dir=last_dir)
            
            if dialog.exec():
                # 添加选中的章节文件夹
                selected_files = dialog.get_selected_chapters()
                if selected_files:
                    self.controller.add_files(selected_files)
                    new_dir = os.path.dirname(selected_files[0])
                    self.controller.set_last_open_dir(new_dir)
        except Exception as e:
            print(f"打开高级文件夹选择失败: {e}")
    ...
    def _toggle_log_collapsed(self):
        ...
        if self._log_expanded:
            ...
            # 收缩：保留80px高度（可看到少量日志）
            self.right_splitter.setSizes([700, 80])
            self.log_collapse_btn.setText("▲")
            self.log_collapse_btn.setToolTip(self._t("Expand Log"))
        
        # 保存状态到配置
        self._monitor_config['log_expanded'] = self._log_expanded
        self._save_monitor_config()
    
    def _save_monitor_config(self):
        """保存监控配置到配置文件"""
        config = self.config_service.get_config()
        config.app.monitor_config = self._monitor_config
        self.config_service.set_config(config)
        self.config_service.save_config_file()
    
    def _on_tab_changed(self, index: int):
        """标签页切换时处理日志区域显示"""
        # 自定义标签页有独立的处理结果框，隐藏主日志
        if hasattr(self, 'custom_comic_tab_index') and index == self.custom_comic_tab_index:
            # 隐藏日志区域
